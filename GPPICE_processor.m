...
%% RUN L1 CHAINS
 try
    ...
    filesBulk.filename_L0 = [filesBulk.inputPath filesBulk.inputFiles(~cellfun(@isempty,strfind(aux,'PIC_SIRS_'))).name];
    if isempty(strfind(filesBulk.filename_L0,'PIC_SIRS_')) % added this, cause aresys changed the input filename in last deliveries. We need to be able to load the 2 types of L0
        filesBulk.filename_L0 = [filesBulk.inputPath filesBulk.inputFiles(~cellfun(@isempty,strfind(aux,'CRA_IR_0_'))).name];
    end
    ...
    if (cnf.run_L1A) %% ------------------------- L1A ---------GO!!!----------------
        ...
        L0_mode_find = strfind(filesBulk.filename_L0, 'PIC_SIRS___');
        
        filesBulk.filename_L1A = strcat(filesBulk.outputPath,'PICE_SIRS_', ...
    ...
    if(~cnf.writting_flag(3)||~cnf.run_DDP)
        ...
        L1A_mode_find = strfind(filesBulk.filename_L1A, 'PICE_SIRS_');
        
        
        filesBulk.filename_L1B = strcat(filesBulk.outputPath,'PICE_SIRS_',...
    ...
    filesBulk.filename_L1BFF_SL = strcat(filesBulk.outputPath,'PICE_SIRS_FF_SL_',...
        filesBulk.filename_L1A((L1A_mode_find(1)+9):(L1A_mode_find(end)+13)), '_1B_',...
        filesBulk.filename_L1A((L1A_date_find(1)+1):(L1A_date_find(1)+30)),...
        '_isd_long','.nc');
    filesBulk.filename_L1BFF_ML = strcat(filesBulk.outputPath,'PICE_SIRS_FF_ML_',...
        filesBulk.filename_L1A((L1A_mode_find(1)+9):(L1A_mode_find(end)+13)), '_1B_',...
        filesBulk.filename_L1A((L1A_date_find(1)+1):(L1A_date_find(1)+30)),...
        '_isd_long','.nc');

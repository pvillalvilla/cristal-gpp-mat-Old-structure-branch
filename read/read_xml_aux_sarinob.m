function strout=read_xml_aux_sarinob(xmlfile)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% --------------------------------------------------------
% Created by isardSAT 
% --------------------------------------------------------
...
%
% ---------------------------------------------------------
% Objective: read CRISTAL SARIN OB CHD/CNF/CST XML auxiliary file
%
% Calling: 
% INPUTs: xmlfile
    ...
    if strcmp(structxml.Children(4).Children(i_par).Name,'Parameter')
        ...
        for i_atr = 1:length(structxml.Children(4).Children(i_par).Children)
            if ~strcmp(structxml.Children(4).Children(i_par).Children(i_atr).Name,'#text') && ~isempty(structxml.Children(4).Children(i_par).Children(i_atr).Children) % if there is data to read
                ...
                elseif (strcmp(structxml.Children(4).Children(i_par).Children(i_atr).Attributes.Name,'mode') && strcmp(structxml.Children(4).Children(i_par).Children(i_atr).Attributes.Value,'OB')) || ... % mode = OB
                        (strcmp(structxml.Children(4).Children(i_par).Children(i_atr).Attributes.Name,'mode') && strcmp(structxml.Children(4).Children(i_par).Children(i_atr).Attributes.Value,'SARIN OB'))    % mode = SARIN OB
                    strtemp(par_count).(structxml.Children(4).Children(i_par).Children(i_atr).Name) = structxml.Children(4).Children(i_par).Children(i_atr).Children.Data;
                end
            end
        end

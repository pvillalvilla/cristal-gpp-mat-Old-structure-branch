...
function [filesBulk] = FFt_chain_zerotimeapproach(filesBulk, chd, cnf, cst, options)
%% INIT VARIABLES
...
[time_pulse,win_delay_pulse,pri_pulse,T0_pulse,Process_ID_pulse,...
    L1A_buffer] = ...
    datation_win_delay_pulse_zerotimeapproach(L1A_buffer, cnf, chd, cst);
...
[x_sar_sat_pulse, y_sar_sat_pulse, z_sar_sat_pulse,...
    ...
    osv_attitude_pulse_zerotimeapproach(L1A_buffer, time_pulse, cnf, chd, cst);
...
if cnf.FFt.num_pools~=1
    ...
    parfor i_surf = 1: N_surfaces 
        ...
        [wfm_AC(i_surf,:), wfm_RC_phase_corr_masked] = ...
        ...
        if(strcmp(cnf.processing_mode,'SIN'))
            [wfm_AC_2(i_surf,:), wfm_RC_phase_corr_masked_2] = ...
                backprojection_kernel_freq_domain(...
...
%             pri_sar_pre_dat_pulse, 1, 1);
 ...
 if ~cnf.trp_flag
     ...
     disp(['Multilooking...']);   
     if(strcmp(cnf.processing_mode,'SIN'))
            [pow_ML_FF, ~, pow_ML_FF_2, ~, phase_difference_ML, wvfm_coh_ML,... 
             ...
             ...
             = multilooking_FF ...
             (wfm_AC, wfm_AC_2, phase_difference, wvfm_coh, ...
             ...
             ProcessID,chd.i_sample_start,cnf.rmc_margin,cnf.processing_mode);

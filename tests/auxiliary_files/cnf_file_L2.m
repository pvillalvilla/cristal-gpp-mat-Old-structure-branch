...
cnf_L2.mode              =   'SAR';      % this shall be LRM, SAR, SARin --> as in CS2 or LRM(1), HRM_raw(2) and HRM_rmc(3) for Jason-CS/S6
if (cnf.onboard_reversion_flag)
    cnf_L2.mode = 'RMC';
end
cnf_L2.Nrmc              =   240/2;    % This will only be used in HRM_rmc processing
...
%----------------------- REF. SAMPLES ------------------------------------
%-------------------------------------------------------------------------
cnf_L2.ref_sample_wd=chd.N_samples_sar/2;
...
if strcmp(cnf_L2.mission,'CS2') || strcmp(cnf_L2.mission,'CR2')
    cnf_L2.Neff_thres        =   chd.N_samples_sar*0.95;    % we tolerate up to 5% of missing echoes max
    if cnf_L2.L1proc == 2
...
%--------------------------------------------------------------------------
cnf_L2.ZP                =   cnf.zp_fact_range;              % 1 if no ZP, 2 if ZP of 2, 4 if ZP of 4, etc...
...
if any(strcmp(cnf_L2.retracker_name,'OCOG'))
   ...
   cnf_L2.OCOG_retracker.n2=chd.N_samples_sar*cnf_L2.ZP; %last zero-padded sample to be used in leading edge estimation
...
if any(strcmp(cnf_L2.retracker_name,'ANALYTICAL') | strcmp(cnf_L2.retracker_name,'SAMOSA'))       
    ...
    cnf_L2.two_step_fitting_COR_threshold_rou=90.0; %threshold below which second fitting on roughness is used
    ...
    switch cnf_L2.fitting_fun_type
        case 'lsq'
            ...
            cnf_L2.fitting_options    = optimset('Algorithm',cnf_L2.lsq_algorithm,'Display','off','UseParallel','always');
    ...
    cnf_L2.plot_fits_downsampling =20;

...
function GPPICE_processor_zerotimeapproach(input_folder, aux_folder, output_folder, options)
...
%% READ CHD
filesBulk.CHD_file=[filesBulk.auxPath filesBulk.auxFiles(~cellfun(@isempty,strfind(aux,'_simulation_description'))).name];
...
%% RUN L1 CHAINS
 try
    ...
    %% ------------------------- FFt ---------GO!!!----------------
    if (cnf.run_FFtime)
		filesBulk = FFt_chain_zerotimeapproach (filesBulk, chd, cnf, cst, options);

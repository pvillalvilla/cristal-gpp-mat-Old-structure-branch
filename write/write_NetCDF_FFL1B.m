...
% 2.3 nb_stack_l1b_echo is L1B.N_beams_contributing instead of L1B.N_beams_stack
% 2.4 added cnf flag cnf.processing_mode
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% NetCDF utils

function write_NetCDF_L1B(files,L1A_buffer,L1B,i_surf_stacked,type,cnf,chd,cst)
%{
...
%[~,beam_index_nadir]=min(abs(-pi/2+L1B.beam_ang_surf'));


burst_index_nadir = 1;% We take the first burst just to test the code (not real). % L1B.burst_index(beam_index_nadir);
...
%}
T0_sar_surf_nadir = L1B.T0_sar_surf; % No L1B.T0_sar_surf(beam_index_nadir) because it is constant!
...
time_l1b_echo = double(L1B.time_surf);
...
UTC_day_l1b_echo = int16(floor(L1B.time_surf./ cst.sec_in_day));
...
if(strcmp(type,'FF-ML'))
    time_surf_ML = double(L1B.time_surf_ML);
    
    % add leap seconds to the TAI time. Only valid for
    if(time_surf_ML < TAI_2012)
        time_surf_ML = time_surf_ML - 34;
    elseif(time_surf_ML > TAI_2015)
        time_surf_ML = time_surf_ML - 36;
    else
        time_surf_ML = time_surf_ML - 35;
    end
end

%----------B. Orbit and attitude variables ------------------------------
lat_l1b_echo = int32(L1B.lat_sat * 1e6);
lon_l1b_echo = int32(L1B.lon_sat * 1e6); 
alt_l1b_echo = int32((L1B.alt_sat-700000) * 1e4); 
orb_alt_rate_l1b_echo = int16(L1B.alt_rate_sat * 1e2);

satellite_mispointing_l1b_sar_echo_ku = int32([L1B.pitch_surf * 180/cst.pi * 1e7; L1B.roll_surf * 180/cst.pi * 1e7; L1B.yaw_surf * 180/cst.pi * 1e7]);

if(strcmp(type,'FF-ML'))
    lat_surf_ML = int32(L1B.lat_surf_ML * 1e6);
    lon_surf_ML = int32(L1B.lon_surf_ML * 1e6);
    alt_surf_ML = int32((L1B.alt_sat_surf_ML-700000) * 1e4);
    alt_rate_surf_ML = int16(L1B.alt_rate_sat_surf_ML * 1e2);
end

%----------C. Flag time variables --------------------------------------

%----------D. Position/Velocity variables ------------------------------
x_pos_l1b_echo=double(L1B.x_sat');
y_pos_l1b_echo=double(L1B.y_sat');
z_pos_l1b_echo=double(L1B.z_sat');

x_vel_l1b_echo=double(L1B.x_vel_sat');
y_vel_l1b_echo=double(L1B.y_vel_sat');
z_vel_l1b_echo=double(L1B.z_vel_sat');

if(strcmp(type,'FF-ML'))  
    x_vel_sat_surf_ML=double(L1B.x_vel_sat_surf_ML');
    y_vel_sat_surf_ML=double(L1B.y_vel_sat_surf_ML');
    z_vel_sat_surf_ML=double(L1B.z_vel_sat_surf_ML');   
end
...
range_ku_l1b_echo=int32((L1B.win_delay_surf*cst.c/2-700000)*1e4);
if cnf.include_wfms_aligned
    %EM:14.04.2016
    range_ku_surf_aligned_l1b_echo=int32((L1B.win_delay_surf_aligned*cst.c/2-700000)*1e4);
end

if(strcmp(type,'FF-ML'))
    range_ML=int32((L1B.win_delay_surf_ML*cst.c/2-700000)*1e4);
end
%{
...
% NOTE: No need to compute stack in FF
% % switch netcdf_type
% %     case 'netcdf4'
% %         nb_stack_l1b_echo=uint16(L1B.N_beams_stack');
% %     case 'netcdf3'
% %         nb_stack_l1b_echo=int32(L1B.N_beams_stack');
% % end
% nb_stack_l1b_echo=int32(L1B.N_beams_contributing);
% nb_stack_start_stop_l1b_echo=int32(L1B.N_beams_start_stop');
% %     aux=squeeze(L1B.beams_rng_cmpr(1:L1B.N_beams_stack,:));
% %     finite_indx=isfinite(aux);
% %     max_stack_l1b_echo = uint32(max(aux(finite_indx))*1e2);
% %     clear aux;
% if L1B.start_beam~=0
%     look_angle_start_l1b_echo = int16(L1B.look_ang_surf(L1B.start_beam)'*1e6);
%     look_angle_stop_l1b_echo = int16(L1B.look_ang_surf(L1B.stop_beam)*1e6);
%     doppler_angle_start_l1b_echo = int16((L1B.doppler_ang_surf(L1B.start_beam))'*1e6);
%     doppler_angle_stop_l1b_echo  = int16((L1B.doppler_ang_surf(L1B.stop_beam))'*1e6);
%     pointing_angle_start_l1b_echo = int16(L1B.pointing_ang_surf(L1B.start_beam)*1e6)';
%     pointing_angle_stop_l1b_echo  = int16(L1B.pointing_ang_surf(L1B.stop_beam)'*1e6);
% else
...
pri_l1b_echo = int32(pri_sar_surf  * 1e12);
...
if(strcmp(type,'FF-SL'))
    ...
    i2q2_meas_ku_l1b_echo = double(round(L1B.wfm_cor_i2q2./ waveform_scale_factor_l1b_echo));
    
    if strcmp(cnf.processing_mode,'SIN')
        waveform_scale_factor_l1b_echo_2 = single(max(L1B.wfm_cor_i2q2_2)/ (2^16-2));
        i2q2_meas_ku_l1b_echo_2 = double(round(L1B.wfm_cor_i2q2_2./ waveform_scale_factor_l1b_echo_2));
...
if strcmp(type, 'FF-ML')
    ...
    %    waveform_scale_factor = single(zeros(N_total_surf_loc,1));
    %    power_waveform        = uint16(zeros(N_total_surf_loc, N_samples_sar_chd * zp_fact_range_cnf));
    %    pnr_estimation        = int16(zeros(N_total_surf_loc,1));
    
    %    pnr_estim_idx = thermal_noise_first_range_bin_cnf:thermal_noise_width_cnf+thermal_noise_first_range_bin_cnf-1;
    
    % M4 waveform scale factor
    waveform_scale_factor = single(max(L1B.pow_ML_FF(:))*10^0.3 / (2^16-2)); % [W]
    % M4 waveform scale factor (conversion to single precision here)
    waveform_scale_factor = single(waveform_scale_factor);
    
    % M10 power waveform
    power_waveform = double(round(L1B.pow_ML_FF(:).*10^0.3 ./ waveform_scale_factor));  % []
    
    if strcmp(cnf.processing_mode,'SIN')
        
        waveform_scale_factor_2 = single(max(L1B.pow_ML_FF_2(:))*10^0.3 / (2^16-2)); % [W]
        % M4 waveform scale factor (conversion to single precision here)
        waveform_scale_factor_2 = single(waveform_scale_factor_2);       
        % M10 power waveform
        power_waveform_2 = double(round(L1B.pow_ML_FF_2(:).*10^0.3 ./ waveform_scale_factor_2));  % []
        
        waveform_scale_factor_comb = single(max(L1B.pow_ML_FF_comb(:))*10^0.3 / (2^16-2)); % [W]
        waveform_scale_factor_comb = single(waveform_scale_factor_comb);       
        % M10 power waveform
        power_waveform_comb = double(round(L1B.pow_ML_FF_comb(:).*10^0.3 ./ waveform_scale_factor_comb));  % []    
        
        phase_diff_ML = double(L1B.phase_difference_ML);
        wvfm_coh_ML = double(L1B.wvfm_coh_ML);
    end
...
if strcmp(type, 'FF-ML')
    var_id = netcdf.inqVarID(files.ncid,'time_surf_ML');
    netcdf.putVar(files.ncid,var_id,i_surf_stacked,time_surf_ML); 
end
...
if strcmp(type, 'FF-ML')
    var_id = netcdf.inqVarID(files.ncid,'lat_surf_ML');
    netcdf.putVar(files.ncid,var_id,i_surf_stacked,lat_surf_ML);
    
    var_id = netcdf.inqVarID(files.ncid,'lon_surf_ML');
    netcdf.putVar(files.ncid,var_id,i_surf_stacked,lon_surf_ML);
    
    var_id = netcdf.inqVarID(files.ncid,'alt_surf_ML');
    netcdf.putVar(files.ncid,var_id,i_surf_stacked,alt_surf_ML);
    
    var_id = netcdf.inqVarID(files.ncid,'alt_rate_surf_ML');
    netcdf.putVar(files.ncid,var_id,i_surf_stacked,alt_rate_surf_ML);
end
...
if strcmp(type, 'FF-ML')
    var_id = netcdf.inqVarID(files.ncid,'x_vel_sat_surf_ML');
    netcdf.putVar(files.ncid,var_id,i_surf_stacked,x_vel_sat_surf_ML);
    
    var_id = netcdf.inqVarID(files.ncid,'y_vel_sat_surf_ML');
    netcdf.putVar(files.ncid,var_id,i_surf_stacked,y_vel_sat_surf_ML);
    
    var_id = netcdf.inqVarID(files.ncid,'z_vel_sat_surf_ML');
    netcdf.putVar(files.ncid,var_id,i_surf_stacked,z_vel_sat_surf_ML);  
end
...
if strcmp(type, 'FF-ML')
    var_id = netcdf.inqVarID(files.ncid,'range_ML');
    netcdf.putVar(files.ncid,var_id,i_surf_stacked,range_ML);
end
...
%------------- M. Waveform related variables -----------------------------
if(strcmp(type,'FF-ML'))
    var_id = netcdf.inqVarID(files.ncid,'power_waveform');
    netcdf.putVar(files.ncid,var_id,[0 i_surf_stacked],[chd.N_samples_sar*cnf.zp_fact_range 1], power_waveform);
    %ncwrite(files.filename_netCDF,'i2q2_meas_ku_l1b_echo',i2q2_meas_ku_l1b_echo.',[1 i_surf_stacked]);
    
    var_id = netcdf.inqVarID(files.ncid,'waveform_scale_factor');
    netcdf.putVar(files.ncid,var_id,i_surf_stacked,waveform_scale_factor);
    
%     var_id = netcdf.inqVarID(files.ncid,'scale_factor_ku_l1b_echo');
%     netcdf.putVar(files.ncid,var_id,i_surf_stacked,scale_factor_ku_l1b_echo);
    
    if strcmp(cnf.processing_mode,'SIN')
        var_id = netcdf.inqVarID(files.ncid,'power_waveform_2');
        netcdf.putVar(files.ncid,var_id,[0 i_surf_stacked],[chd.N_samples_sar*cnf.zp_fact_range 1], power_waveform_2);
        
        var_id = netcdf.inqVarID(files.ncid,'waveform_scale_factor_2');
        netcdf.putVar(files.ncid,var_id,i_surf_stacked,waveform_scale_factor_2);
        
        var_id = netcdf.inqVarID(files.ncid,'power_waveform_comb');
        netcdf.putVar(files.ncid,var_id,[0 i_surf_stacked],[chd.N_samples_sar*cnf.zp_fact_range 1], power_waveform_comb);
        
        var_id = netcdf.inqVarID(files.ncid,'waveform_scale_factor_comb');
        netcdf.putVar(files.ncid,var_id,i_surf_stacked,waveform_scale_factor_comb);        
        
        var_id = netcdf.inqVarID(files.ncid,'phase_diff_ML');
        netcdf.putVar(files.ncid,var_id,[0 i_surf_stacked],[chd.N_samples_sar*cnf.zp_fact_range 1], phase_diff_ML.');
        
        var_id = netcdf.inqVarID(files.ncid,'wvfm_coh_ML');
        netcdf.putVar(files.ncid,var_id,[0 i_surf_stacked],[chd.N_samples_sar*cnf.zp_fact_range 1], wvfm_coh_ML.');
    end
end
...
% if strcmp(cnf.mode,'SIN') && strcmp(cnf.processing_mode,'SIN') % && strcmp(mission,'CR2')
%     var_id = netcdf.inqVarID(files.ncid,'i2q2_meas_ku_l1b_echo_2');

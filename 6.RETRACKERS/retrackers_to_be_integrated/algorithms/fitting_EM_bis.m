function fit_res = fitting_EM_bis(data, cnf_p, nf_p, ini_p,varargin)
...
switch cnf_p.range_index_method
    case 'conventional'
        range_index=1:data.N_samples;
    case 'resampling'
        range_index=interp((1:data.N_samples/cnf_p.ZP),cnf_p.ZP);
end
                ...
                switch cnf_p.power_wfm_model
                    case 'simple'
                        switch cnf_p.fitting_fun_type
                            ...
                            case 'fmin'
                                fminfun               =   @(fit_params)sum((ml_wave_gen_EM(range_index, fit_params, nf_params, cnf_p,looks,func_f0)-fit_data).^2);
                        end
                    ...
                    case 'complete'
                        switch cnf_p.fitting_fun_type
                            ...
                            case 'fmin'
                                fminfun               =   @(fit_params)sum((ml_wave_gen_EM(range_index, fit_params, nf_params, cnf_p,looks,func_f0,func_f1)-fit_data).^2);
                        end
                end
            else
                switch cnf_p.fitting_fun_type
...
%             end
            ...
            if cnf_p.lut_flag
                switch cnf_p.power_wfm_model
                    case 'simple'
                        ml_wav          =   ml_wave_gen_EM(range_index,fit_params,nf_params, cnf_p,looks,func_f0);
                        
                    case 'complete'
                        ml_wav          =   ml_wave_gen_EM(range_index,fit_params,nf_params, cnf_p,looks,func_f0,func_f1);
                end
            else
                ml_wav          =   ml_wave_gen_EM(range_index,fit_params,nf_params, cnf_p,looks);
            end

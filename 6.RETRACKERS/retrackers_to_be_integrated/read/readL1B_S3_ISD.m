function data = readL1B_S3_ISD (filename_L1B,cnf_p,varargin)
% -------------------------------------------------------------------------
...
% COMMENTS: 
%   - The first version v1 is exactly the same as readL1B_CS2_ISD which
%   exploits the netCDF definition of L1B used for the different SEOM
%   projects and based on the Sentinel-3 formating
% -------------------------------------------------------------------------
% -------------------------------------------------------------------------
% INPUT:
%       MANDATORY:
...
% -------------------------------------------------------------------------
% Versions control:
% v1.0: Based on readL1B_CS2_ISD.m
       ...
       %% --------------------- NetCDF ---------------------------------------
        ...
        switch cnf_p.mode
            case {'SAR','HR'}
                ...
                data.HRM.pri_surf=(double(ncread(filename_L1B,'pri_lrm_l1b_echo_sar_ku')).*scale_factor).'; %in seconds
...
%         data.GLOBAL_ATT.ORBIT_INFO.Equator_Cross_Long=ncreadatt(filename_L1B,'/','orbit_Equator_Cross_Long');

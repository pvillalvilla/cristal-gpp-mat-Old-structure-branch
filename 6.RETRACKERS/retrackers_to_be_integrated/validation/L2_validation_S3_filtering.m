...
function [res]=L2_validation_S3_filtering(filename_L2_ISD,filename_L2_ESA,path_results_comparison,varargin)
global generate_kml
%==========================================================================
...
name_file_L2_ISD=strsplit(char(name_file_L2_ISD(end)),'.nc');
name_file_L2_ISD=char(name_file_L2_ISD(1));
...
%% ------------------- Read L2 ESA product --------------------------------
%-------------- Geometry parameters ---------------------------------------
ESA_lat_surf=double(ncread(filename_L2_ESA,'lat_20_ku')).';
ESA_lon_surf=double(ncread(filename_L2_ESA,'lon_20_ku')).';
ESA_num_surfaces=length(ESA_lat_surf);

%-------------- Geophysical parameters ------------------------------------
%Using the ocean retracker
ESA_L2_range_corr=double(ncread(filename_L2_ESA,'range_ocean_20_ku')).';
ESA_L2_alt=double(ncread(filename_L2_ESA,'alt_20_ku')).';
ESA_L2_SSH_nocorr=ESA_L2_alt-ESA_L2_range_corr;
ESA_L2_sigma0=double(ncread(filename_L2_ESA,'sig0_ocean_20_ku')).';
ESA_L2_SWH=double(ncread(filename_L2_ESA,'swh_ocean_20_ku')).';
...
idx_outliers_ESA_L2_SWH=zeros(1,ESA_num_surfaces_filtered);
...
if flag_outliers_removal
    ...
    switch type_outliers_removal
        case 'percentiles'
            [ESA_L2_SSH_nocorr(ESA_indexes_int),idx_outliers_ESA_L2_SSH]=outliers_by_percentiles(ESA_L2_SSH_nocorr(ESA_indexes_int),outlier_percentil_low,outlier_percentil_high,IQR_times);
            ESA_L2_SSH_nocorr(idx_int_ESA(idx_outliers_ESA_L2_SSH))=NaN;            
        case 'hampel'
            %[ESA_L2_SSH_r1(ESA_indexes_int)] = hampel(ESA_lat_surf(ESA_indexes_int),ESA_L2_SSH_r1(ESA_indexes_int),hampel_wind,hampel_sigma);            
            [~,idx_outliers_ESA_L2_SSH] = hampel(ESA_lat_surf(ESA_indexes_int),ESA_L2_SSH_nocorr(ESA_indexes_int),hampel_wind,hampel_sigma);            
            ESA_L2_SSH_nocorr(idx_int_ESA(idx_outliers_ESA_L2_SSH))=NaN;
    end
    % ----------------------------- SWH ---------------------------------------
    switch type_outliers_removal
        case 'percentiles'
            [ESA_L2_SWH(ESA_indexes_int),idx_outliers_ESA_L2_SWH]=outliers_by_percentiles(ESA_L2_SWH(ESA_indexes_int),outlier_percentil_low,outlier_percentil_high,IQR_times);
            ESA_L2_SWH(idx_int_ESA(idx_outliers_ESA_L2_SWH))=NaN;            
        case 'hampel'
            %[ESA_L2_SSH_r1(ESA_indexes_int)] = hampel(ESA_lat_surf(ESA_indexes_int),ESA_L2_SSH_r1(ESA_indexes_int),hampel_wind,hampel_sigma);            
            [~,idx_outliers_ESA_L2_SWH] = hampel(ESA_lat_surf(ESA_indexes_int),ESA_L2_SWH(ESA_indexes_int),hampel_wind,hampel_sigma);            
            ESA_L2_SWH(idx_int_ESA(idx_outliers_ESA_L2_SWH))=NaN;
    end
	% ----------------------------- SIGMA0 ------------------------------------
    switch type_outliers_removal
        case 'percentiles'
            [ESA_L2_sigma0(ESA_indexes_int),idx_outliers_ESA_L2_sigma0]=outliers_by_percentiles(ESA_L2_sigma0(ESA_indexes_int),outlier_percentil_low,outlier_percentil_high,IQR_times);            
        case 'hampel'
            ...
            [~,idx_outliers_ESA_L2_sigma0] = hampel(ESA_lat_surf(ESA_indexes_int),ESA_L2_sigma0(ESA_indexes_int),hampel_wind,hampel_sigma);
            ESA_L2_sigma0(idx_int_ESA(idx_outliers_ESA_L2_sigma0))=NaN;
    end
...
for i_wfm=1:ESA_num_surfaces_filtered
    ...
    % end    
    ESA_L2_SSH_smoothed(i_wfm) = nanmean(ESA_L2_SSH_nocorr(idx_int_ESA(start:finish)));
    

    %------------------------------ SWH -----------------------------------
    start               = max(i_wfm-sliding_window_SWH,1);
    finish              = min(i_wfm+sliding_window_SWH-1,ESA_num_surfaces_filtered);
    % mean_win            = nanmean(ESA_L2_SSH_r1(idx_int_ESA(start:finish)));
    % std_win             = nanstd(ESA_L2_SSH_r1(idx_int_ESA(start:finish)));    
    % if ESA_L2_SSH_r1(idx_int_ESA(i_wfm))>(mean_win+threshold_std*std_win) || ESA_L2_SSH_r1(idx_int_ESA(i_wfm))<(mean_win-threshold_std*std_win)
        % idx_outliers_ESA_L2_SSH(i_wfm) = 1;
    % end    
    ESA_L2_SWH_smoothed(i_wfm) = nanmean(ESA_L2_SWH(idx_int_ESA(start:finish)));
    ...
    % end
    ESA_L2_sigma0_smoothed(i_wfm) = nanmean(ESA_L2_sigma0(idx_int_ESA(start:finish)));
end
...
%remove outliers:
ESA_L2_SSH_filtered=ESA_L2_SSH_nocorr(ESA_indexes_int);
...
if ESA_num_surfaces_filtered==ISD_num_surfaces_filtered
    ...
    if ~isempty(idx_int_analytical_SWH_MSSfixed)
        ...
        res.SSH.ANALYTICAL_SWH_MSSfixed.RMSE_error_L2=sqrt(nanmean((ESA_L2_SSH_filtered(~idx_outliers)-ISD_L2_SSH_analytical_SWH_MSSfixed_filtered(~idx_outliers)).^2));
        ...
        res.SSH.ANALYTICAL_SWH_MSSfixed.mean_error_L2=(nanmean((ESA_L2_SSH_filtered(~idx_outliers)-ISD_L2_SSH_analytical_SWH_MSSfixed_filtered(~idx_outliers))));
    ...
    if ~isempty(idx_int_analytical_MSS_SWHfixed)
        ...
        res.SSH.ANALYTICAL_MSS_SWHfixed.RMSE_error_L2=sqrt(nanmean((ESA_L2_SSH_filtered(~idx_outliers)-ISD_L2_SSH_analytical_MSS_SWHfixed_filtered(~idx_outliers)).^2));
        ...
        res.SSH.ANALYTICAL_MSS_SWHfixed.mean_error_L2=(nanmean((ESA_L2_SSH_filtered(~idx_outliers)-ISD_L2_SSH_analytical_MSS_SWHfixed_filtered(~idx_outliers))));
    ...
    if ~isempty(idx_int_thres)
        ...
        res.SSH.THRESHOLD.RMSE_error_L2=sqrt(nanmean((ESA_L2_SSH_filtered(~idx_outliers)-ISD_L2_SSH_threshold_filtered(~idx_outliers)).^2));
        fprintf(fid,'RMSE error SSH ESA-ISD (threshold-retracker) [m]: '); fprintf(fid,'%.18g\n',res.SSH.THRESHOLD.RMSE_error_L2);
        res.SSH.THRESHOLD.mean_error_L2=(nanmean((ESA_L2_SSH_filtered(~idx_outliers)-ISD_L2_SSH_threshold_filtered(~idx_outliers))));
    ...
    if ~isempty(idx_int_ocog)
        ...
        res.SSH.OCOG.RMSE_error_L2=sqrt(nanmean((ESA_L2_SSH_filtered(~idx_outliers)-ISD_L2_SSH_OCOG_filtered(~idx_outliers)).^2));
        fprintf(fid,'RMSE error SSH ESA-ISD (OCOG-retracker) [m]: '); fprintf(fid,'%.18g\n',res.SSH.OCOG.RMSE_error_L2);
        res.SSH.OCOG.mean_error_L2=(nanmean((ESA_L2_SSH_filtered(~idx_outliers)-ISD_L2_SSH_OCOG_filtered(~idx_outliers))));
...
res.SSH.ESA_L2.mean_error_fitting=nanmean(ESA_L2_SSH_filtered(~idx_outliers_ESA_L2_SSH)-ESA_L2_SSH_smoothed(~idx_outliers_ESA_L2_SSH));
fprintf(fid,'Mean error SSH fitting (L2-ESA) [m]: '); fprintf(fid,'%.18g\n',res.SSH.ESA_L2.mean_error_fitting);
res.SSH.ESA_L2.rmse_fitting=sqrt(nanmean((ESA_L2_SSH_filtered(~idx_outliers_ESA_L2_SSH)-ESA_L2_SSH_smoothed(~idx_outliers_ESA_L2_SSH)).^2));
...
[counts,centers_ESA_L2_SSH_r1_filtered]=hist(ESA_L2_SSH_filtered(~idx_outliers_ESA_L2_SSH),nbins);
...
figure;  
plot(ESA_lat_surf(ESA_indexes_int),ESA_L2_SSH_nocorr(ESA_indexes_int),linestyle_ESA);
hold on;
legend_text={'L2-IPF'};
text_in_textbox={''};
text_in_textbox=[text_in_textbox,...
...
fprintf(fid,'$---------------- SIGMA0  --------------------------------------------$\n');
fprintf(fid,'$---------------- ------  --------------------------------------------$\n');
ESA_L2_sigma0_filtered=ESA_L2_sigma0(ESA_indexes_int);
%idx_outliers_ESA_L2_SSH=zeros(1,ESA_num_surfaces_filtered);
if ESA_num_surfaces_filtered==ISD_num_surfaces_filtered
    fprintf(fid,'$---------------- ---------- ---------- --- -------------------------------------------$\n');
    fprintf(fid,'$---------------- RETRACKERS COMPARISON ESA -------------------------------------------$\n');
    ...
    if ~isempty(idx_int_analytical_SWH_MSSfixed)
        ...
        res.SIGMA0.ANALYTICAL_SWH_MSSfixed.RMSE_error_L2=sqrt(nanmean((ESA_L2_sigma0_filtered(~idx_outliers)-ISD_L2_sigma0_analytical_SWH_MSSfixed_filtered(~idx_outliers)).^2));
        ...
        res.SIGMA0.ANALYTICAL_SWH_MSSfixed.mean_error_L2=(nanmean((ESA_L2_sigma0_filtered(~idx_outliers)-ISD_L2_sigma0_analytical_SWH_MSSfixed_filtered(~idx_outliers))));
    ...
    if ~isempty(idx_int_analytical_MSS_SWHfixed)
        ...
        res.SIGMA0.ANALYTICAL_MSS_SWHfixed.RMSE_error_L2=sqrt(nanmean((ESA_L2_sigma0_filtered(~idx_outliers)-ISD_L2_sigma0_analytical_MSS_SWHfixed_filtered(~idx_outliers)).^2));
        ...
        res.SIGMA0.ANALYTICAL_MSS_SWHfixed.mean_error_L2=(nanmean((ESA_L2_sigma0_filtered(~idx_outliers)-ISD_L2_sigma0_analytical_MSS_SWHfixed_filtered(~idx_outliers))));
    ...
    if ~isempty(idx_int_thres)
        ...
        res.SIGMA0.THRESHOLD.RMSE_error_L2=sqrt(nanmean((ESA_L2_sigma0_filtered(~idx_outliers)-ISD_L2_sigma0_threshold_filtered(~idx_outliers)).^2));
        fprintf(fid,'RMSE error sigma0 ESA-ISD (threshold-retracker) [dB]: '); fprintf(fid,'%.18g\n',res.SIGMA0.THRESHOLD.RMSE_error_L2);
        res.SIGMA0.THRESHOLD.mean_error_L2=(nanmean((ESA_L2_sigma0_filtered(~idx_outliers)-ISD_L2_sigma0_threshold_filtered(~idx_outliers))));
    ...
    if ~isempty(idx_int_ocog)
        ...
        res.SIGMA0.OCOG.RMSE_error_L2=sqrt(nanmean((ESA_L2_sigma0_filtered(~idx_outliers)-ISD_L2_sigma0_OCOG_filtered(~idx_outliers)).^2));
        fprintf(fid,'RMSE error sigma0 ESA-ISD (OCOG-retracker) [dB]: '); fprintf(fid,'%.18g\n',res.SIGMA0.OCOG.RMSE_error_L2);
        res.SIGMA0.OCOG.mean_error_L2=(nanmean((ESA_L2_sigma0_filtered(~idx_outliers)-ISD_L2_sigma0_OCOG_filtered(~idx_outliers))));
...
fprintf(fid,'$---------------- ESA RETRACKER -------------------------------------------$\n');
res.SIGMA0.ESA_L2.mean_error_fitting=nanmean(ESA_L2_sigma0_filtered(~idx_outliers_ESA_L2_sigma0)-ESA_L2_sigma0_smoothed(~idx_outliers_ESA_L2_sigma0));
fprintf(fid,'Mean error sigma0 fitting (L2-ESA) [dB]: '); fprintf(fid,'%.18g\n',res.SIGMA0.ESA_L2.mean_error_fitting);
res.SIGMA0.ESA_L2.rmse_fitting=sqrt(nanmean((ESA_L2_sigma0_filtered(~idx_outliers_ESA_L2_sigma0)-ESA_L2_sigma0_smoothed(~idx_outliers_ESA_L2_sigma0)).^2));
...
[counts,centers_ESA_L2_sigma0_r1_filtered]=hist(ESA_L2_sigma0_filtered(~idx_outliers_ESA_L2_sigma0),nbins);
...
%---------------- Plot comparison ESA & ISD -------------------------------
figure;  
plot(ESA_lat_surf(ESA_indexes_int),ESA_L2_sigma0(ESA_indexes_int),linestyle_ESA);
hold on;
legend_text={'L2-IPF'};
text_in_textbox={''};
text_in_textbox=[text_in_textbox,...
...
if ESA_num_surfaces_filtered==ISD_num_surfaces_filtered
    ...
    fprintf(fid,'$---------------- RETRACKERS COMPARISON ESA -------------------------------------------$\n');
    fprintf(fid,'$---------------- ---------- ---------- --- -------------------------------------------$\n');
    ESA_L2_SWH_filtered=ESA_L2_SWH(ESA_indexes_int);
    %----------------- analytical_SWH_MSSfixed -------------------------------------
    if ~isempty(idx_int_analytical_SWH_MSSfixed)
        fprintf(fid,'$---------------- ANALYTICAL_SWH_MSSfixed RETRACKER -------------------------------------------$\n');
        %----------- Comparison with ESA L2 ---------------------------        
		idx_outliers=idx_outliers_ESA_L2_SWH | idx_outliers_ISD_L2_SWH_analytical_SWH_MSSfixed;
        res.SWH.ANALYTICAL_SWH_MSSfixed.RMSE_error_L2=sqrt(nanmean((ESA_L2_SWH_filtered(~idx_outliers)-ISD_L2_SWH_analytical_SWH_MSSfixed_filtered(~idx_outliers)).^2));
        fprintf(fid,'RMSE error SWH ESA-ISD (analytical_SWH_MSSfixed-retracker) [m]: '); fprintf(fid,'%.18g\n',res.SWH.ANALYTICAL_SWH_MSSfixed.RMSE_error_L2);
        res.SWH.ANALYTICAL_SWH_MSSfixed.mean_error_L2=(nanmean((ESA_L2_SWH_filtered(~idx_outliers)-ISD_L2_SWH_analytical_SWH_MSSfixed_filtered(~idx_outliers))));
        fprintf(fid,'Mean error SWH ESA-ISD (analytical_SWH_MSSfixed-retracker) [m]: '); fprintf(fid,'%.18g\n',res.SWH.ANALYTICAL_SWH_MSSfixed.mean_error_L2);        
    end
end
fprintf(fid,'$---------------- ---------- ------- ------ -------------------------------------------$\n');
fprintf(fid,'$---------------- RETRACKERS FITTING ERRORS -------------------------------------------$\n');
...
fprintf(fid,'$---------------- ESA ANALYTICAL RETRACKER -------------------------------------------$\n');
res.SWH.ANALYTICAL_STL.mean_error_fitting=nanmean(ESA_L2_SWH_filtered(~idx_outliers_ESA_L2_SWH)-ESA_L2_SWH_smoothed(~idx_outliers_ESA_L2_SWH));
fprintf(fid,'Mean error SWH fitting (analytical ESA) [m]: '); fprintf(fid,'%.18g\n',res.SWH.ANALYTICAL_STL.mean_error_fitting);
res.SWH.ANALYTICAL_STL.rmse_fitting=sqrt(nanmean((ESA_L2_SWH_filtered(~idx_outliers_ESA_L2_SWH)-ESA_L2_SWH_smoothed(~idx_outliers_ESA_L2_SWH)).^2));
fprintf(fid,'RMSE SWH fitting (analytical ESA) [m]: '); fprintf(fid,'%.18g\n',res.SWH.ANALYTICAL_STL.rmse_fitting);
...
legend_text={''};
text_in_textbox={''};
    plot(ESA_lat_surf(ESA_indexes_int),ESA_L2_SWH(ESA_indexes_int),linestyle_ESA)
    hold on;
    plot(ESA_lat_surf(ESA_indexes_int),abs(ESA_L2_SWH_smoothed),linestyle_ESA_smooth);
    legend_text=[legend_text,{'L2-IPF','L2-IPF (smoothed)'}];
    text_in_textbox=[text_in_textbox,...
        strcat({'ESA L2 '},' --> RMSE [m]:',{' '},num2str(res.SWH.ANALYTICAL_STL.rmse_fitting,'%.4g'))];%,', Mean [m]:',{' '},num2str(res.SWH.ANALYTICAL_STL.mean_error_fitting,'%.4g'))];
    title(strcat('Comparison SWH: isardSAT & ESA'),'Interpreter','Tex');
if ~isempty(idx_int_analytical_SWH_MSSfixed)

function [SSH_std,SWH_std,sigma0_std,amp_fit_std,...
    ...
    num_surfaces,...
    ISR_ESA_SSH_std,ISR_ESA_sigma0_std,ISR_ESA_SSH_std_mean,...
    ISR_ESA_sigma0_std_mean,ISR_ESA_SSH_mean,ISR_ESA_sigma0_mean,...
    ISR_ESA_SSH,ISR_ESA_sigma0,...
    SSH_mean_error_ISR_ESA_ISR,sigma0_mean_error_ISR_ESA_ISR,...
    ISR_ESA_SSH_smooth,ISR_ESA_sigma0_smooth,...
    ISR_ESA_SSH_RMSE,ISR_ESA_sigma0_RMSE]=...
                                                run_performance_ISR_baselines(N_baselines,filesBulk,input_path_L2_ISR_bs,input_path_L2_ESA,input_path_L1_ESA,input_path_L2_STL,input_path_L1_ISR_bs,input_path_L2_ISR_L1B_ESA,...
...
if isempty(input_path_L2_ISR_L1B_ESA)
    ISR_ESA_SSH_std=[];
    ISR_ESA_sigma0_std=[];
    ISR_ESA_SSH_std_mean=[];
    ISR_ESA_sigma0_std_mean=[];
    ISR_ESA_SSH_mean=[];
    ISR_ESA_sigma0_mean=[];
    ISR_ESA_SSH=[];
    ISR_ESA_sigma0=[];
    SSH_mean_error_ISR_ESA_ISR=[];
    sigma0_mean_error_ISR_ESA_ISR=[];
    ISR_ESA_SSH_smooth=[];
    ISR_ESA_sigma0_smooth=[];
...
for b=1:N_baselines
    ...
    if b==1
        ...
        %% --------- read the L2 ISR extracted from L1B ESA ----------------
        if ~isempty(input_path_L2_ISR_L1B_ESA)
            input_L2_ISR_ESA_Files   = dir(fullfile(char(input_path_L2_ISR_L1B_ESA),['*' data_string(1:15) '*.nc']));
            if isempty(input_L2_ISR_ESA_Files)
                %add one second to initial time acquisition
                init_acq_time=datestr(datenum((data_string(1:15)),'yyyymmddTHHMMSS')+1/24/60/60,'yyyymmddTHHMMSS');
                input_L2_ISR_ESA_Files   = dir(fullfile(input_path_L2_ISR_L1B_ESA,['*' strcat(init_acq_time,data_string(16:31)) '*.nc']));
                if isempty(input_L2_ISR_ESA_Files)
                    %add one second to initial time acquisition
                    end_acq_time=datestr(datenum((data_string(25:31)),'yyyymmddTHHMMSS')+1/24/60/60,'yyyymmddTHHMMSS');
                    input_L2_ISR_ESA_Files   = dir(fullfile(input_path_L2_ISR_L1B_ESA,['*' strcat(data_string(1:16),end_acq_time) '*.nc']));
                    if isempty(input_L2_ISR_ESA_Files)
                        input_L2_ISR_ESA_Files   = dir(fullfile(input_path_L2_ISR_L1B_ESA,['*' strcat(init_acq_time,'_',end_acq_time) '*.nc']));
                    end
                end
            end
            
            filename_L2_ISR_ESA=strcat(char(input_path_L2_ISR_L1B_ESA),input_L2_ISR_ESA_Files.name);
            
            %-------------- Geometry parameters ---------------------------------------
            ISR_ESA_lat_surf=double(ncread(filename_L2_ISR_ESA,'lat_20_ku')).';
            ISR_ESA_lon_surf=double(ncread(filename_L2_ISR_ESA,'lon_20_ku')).';
            
            if isempty(strfind(lower(char(name_bs_L2_ISR_L1B_ESA)),'thresh'))
                %analytical retracker SAR ocean model
                SSH_dumm=double(ncread(filename_L2_ISR_ESA,strcat(sh_name_nc,'_analytical_SWH_MSSfixed_20_ku'))).';
                SWH_dumm=double(ncread(filename_L2_ISR_ESA,'swh_analytical_SWH_MSSfixed_20_ku')).';
                sigma0_dumm=double(ncread(filename_L2_ISR_ESA,'sig0_analytical_SWH_MSSfixed_20_ku')).';
                COR_dumm=double(ncread(filename_L2_ISR_ESA,'Pearson_corr_analytical_SWH_MSSfixed_20_ku')).';
            else
                %peak retracker
                ISR_ESA_SSH=double(ncread(filename_L2_ISR_ESA,strcat(sh_name_nc,'_threshold_20_ku'))).';
                ISR_ESA_sigma0=double(ncread(filename_L2_ISR_ESA,'sig0_threshold_20_ku')).';
            end
...
%             
            ...
            if ~isempty(input_path_L2_ISR_L1B_ESA)
                ISR_ESA_SSH=ISR_ESA_SSH(ESA_indexes_int);
                ISR_ESA_sigma0=ISR_ESA_sigma0(ESA_indexes_int);
            end
        end
    ...
    if flag_outliers_removal
        switch type_outliers_removal
            case 'percentiles'
                ...
                if ~isempty(input_path_L2_ISR_L1B_ESA) && b==1
                    %------------------- SSH ----------------------------------
                    [ISR_ESA_SSH,idx_outliers_SSH_ISR_ESA]=outliers_by_percentiles(ISR_ESA_SSH,outlier_percentil_low,outlier_percentil_high,IQR_times);
                    idx_nooutliers_SSH_ISR_ESA=find(~idx_outliers_SSH_ISR_ESA);
                    %----------------- sigma0 ---------------------------------
                    [ISR_ESA_sigma0,idx_outliers_sigma0_ISR_ESA]=outliers_by_percentiles(ISR_ESA_sigma0,outlier_percentil_low,outlier_percentil_high,IQR_times);
                    idx_nooutliers_sigma0_ISR_ESA=find(~idx_outliers_sigma0_ISR_ESA);
                end
            ...
            case 'hampel'
                ...
                if ~isempty(input_path_L2_ISR_L1B_ESA) && b==1
                    %------------------ SSH ---------------------------
                    [~,idx_outliers_SSH_ISR_ESA] = hampel(ESA_lat_surf,ISR_ESA_SSH,hampel_wind,hampel_sigma);
                    ISR_ESA_SSH(idx_outliers_SSH_ISR_ESA)=NaN;
                    idx_nooutliers_SSH_ISR_ESA=find(~idx_outliers_SSH_ISR_ESA);
                    %----------------- sigma0 ---------------------------------
                    [~,idx_outliers_sigma0_ISR_ESA] = hampel(ESA_lat_surf,ISR_ESA_sigma0,hampel_wind,hampel_sigma);
                    ISR_ESA_sigma0(idx_outliers_sigma0_ISR_ESA)=NaN;
                    idx_nooutliers_sigma0_ISR_ESA=find(~idx_outliers_sigma0_ISR_ESA);
                end
                if ~isempty(input_path_L2_STL) && b==1
                    %------------------ SSH ---------------------------
    ...
    if smooth_param
        ...
        if ~isempty(input_path_L2_ISR_L1B_ESA) && b==1
            %----------------- SSH ------------------------------------
            ISR_ESA_SSH_smooth=smooth(ISR_ESA_SSH,win_size_detrending).';
            %----------------- SIGMA0 ---------------------------------
            ISR_ESA_sigma0_smooth=smooth(ISR_ESA_sigma0,win_size_detrending).';
        end
    ...
    for i_box=1:(num_boxes+1)
        ...
        if ~isempty(input_path_L2_ISR_L1B_ESA) && b==1
            ISR_ESA_SSH_std(i_box)=nanstd(detrend(ISR_ESA_SSH(idx_nooutliers_SSH_ISR_ESA(idx_nooutliers_SSH_ISR_ESA>=init_sample & idx_nooutliers_SSH_ISR_ESA<=last_sample))));
            ISR_ESA_sigma0_std(i_box)=nanstd(detrend(ISR_ESA_sigma0(idx_nooutliers_sigma0_ISR_ESA(idx_nooutliers_sigma0_ISR_ESA>=init_sample & idx_nooutliers_sigma0_ISR_ESA<=last_sample))));
            ISR_ESA_SSH_mean(i_box)=nanmean((ISR_ESA_SSH(idx_nooutliers_SSH_ISR_ESA(idx_nooutliers_SSH_ISR_ESA>=init_sample & idx_nooutliers_SSH_ISR_ESA<=last_sample))));
            ISR_ESA_sigma0_mean(i_box)=nanmean((ISR_ESA_sigma0(idx_nooutliers_sigma0_ISR_ESA(idx_nooutliers_sigma0_ISR_ESA>=init_sample & idx_nooutliers_sigma0_ISR_ESA<=last_sample))));
        end
    ...
    if smooth_param
        ...
        if ~isempty(input_path_L2_ISR_L1B_ESA) && b==1
            ISR_ESA_SSH_RMSE=sqrt(nanmean((ISR_ESA_SSH-ISR_ESA_SSH_smooth).^2));
            ISR_ESA_sigma0_RMSE=sqrt(nanmean((ISR_ESA_sigma0-ISR_ESA_sigma0_smooth).^2));
        end
    ...
    if ~isempty(input_path_L2_ISR_L1B_ESA) && b==1
        ISR_ESA_SSH_std_mean=nanmean(ISR_ESA_SSH_std);
        ISR_ESA_sigma0_std_mean=nanmean(ISR_ESA_sigma0_std);
    end
    ...
    if ~isempty(input_path_L2_ISR_L1B_ESA)
        SSH_mean_error_ISR_ESA_ISR(b,1)=nanmean(ISR_ESA_SSH_mean-SSH_mean(b,:));
        sigma0_mean_error_ISR_ESA_ISR(b,1)=nanmean(ISR_ESA_sigma0_mean-sigma0_mean(b,:));
    end
...
%--------------------- SSH --------------------------------------------
if generate_plots==1
    if  mod(i_files_valid,plot_downsampling)==0 || i_files_valid==1
        f1=figure;
        legend_text={''};
        text_in_textbox={''};
        text_errors_ESA_ISR={''};
        ...
        if ~isempty(input_path_L2_ISR_L1B_ESA)
            plot(ESA_lat_surf,ISR_ESA_SSH,'Marker',marker_ISR_ESA,'Color',color_ISR_ESA);
            hold on;
            legend_text=[legend_text,name_bs_L2_ISR_L1B_ESA];
            text_in_textbox=[text_in_textbox,...
                strcat(name_bs_L2_ISR_L1B_ESA,{': RMSE [m]: '},num2str(ISR_ESA_SSH_RMSE,'%.4g'),{' , std [m]: '},num2str(ISR_ESA_SSH_std_mean,'%.4g'))];
        end
        ...
        if ~isempty(input_path_L2_ESA)
            ...
            text_in_textbox=[text_in_textbox,...
                strcat(name_bs_L2_ESA,{' : RMSE [m]: '},num2str(ESA_SSH_RMSE,'%.4g'),{' , std [m]: '},num2str(ESA_SSH_std_mean,'%.4g'))];
        end
        ...
        if ~isempty(input_path_L2_STL)
            ...
            text_in_textbox=[text_in_textbox,...
                strcat(name_bs_L2_STL,{': RMSE [m]: '},num2str(STL_SSH_RMSE,'%.4g'),{' , std [m]: '},num2str(STL_SSH_std_mean,'%.4g'))];
        end
        ...
        for b=1:N_baselines
            ...
            text_in_textbox=[text_in_textbox,...
                strcat(name_bs(b),{': RMSE [m]: '},num2str(SSH_RMSE(b,1),'%.4g'),{' , std [m]: '},num2str(SSH_std_mean(b,1),'%.4g'))];

            if ~isempty(input_path_L2_ISR_L1B_ESA)
                text_errors_ESA_ISR=[text_errors_ESA_ISR,...
                    strcat({'Mean error '},name_bs_L2_ISR_L1B_ESA,{' - '},name_bs(b),{' [m]: '},num2str(SSH_mean_error_ISR_ESA_ISR(b,1),'%.4g'))];
            end
            
            if ~isempty(input_path_L2_ESA)
                text_errors_ESA_ISR=[text_errors_ESA_ISR,...
                    strcat({'Mean error '},name_bs_L2_ESA,{' - '},name_bs(b),{' [m]: '},num2str(SSH_mean_error_ESA_ISR(b,1),'%.4g'))];
            end
            
            if ~isempty(input_path_L2_STL)
                text_errors_STL_ISR=[text_errors_STL_ISR,...
                    strcat({'Mean error '},name_bs_L2_STL,{' - '},name_bs(b),{' [m]: '},num2str(SSH_mean_error_STL_ISR(b,1),'%.4g'))];
            end
        end
        ...
        print(print_file,res_fig,strcat(path_comparison_results,data_string,'_cmp_',ext_baselines_comp,'_SSH',file_ext))
        save(strcat(path_comparison_results,data_string,'_cmp_',ext_baselines_comp,'_SSH','.mat'),...
            'input_path_L2_ESA','input_path_L2_ISR_L1B_ESA','ESA_lat_surf','ESA_L2_SSH_r1','marker_ESA','color_ESA','marker_ISR_ESA','color_ISR_ESA',...
            'input_path_L2_STL','ISR_lat_surf','STL_L2_SSH','marker_STL','color_STL',...
            'N_baselines','SSH','marker_bs','color_bs',...
            'name_bs','name_bs_L2_ESA','name_bs_L2_STL','name_bs_L2_ISR_L1B_ESA','sh_name_nc','legend_text','legend_fontsize','text_in_textbox','textbox_fontsize',...
        ...
        if ~isempty(input_path_L2_STL)
            ...
            legend_text=[legend_text,name_bs_L2_STL];
            text_in_textbox=[text_in_textbox,...
                strcat(name_bs_L2_STL,{': RMSE [m]: '},num2str(STL_SWH_RMSE,'%.4g'),{' , std [m]: '},num2str(STL_SWH_std_mean,'%.4g'))];
        end
        for b=1:N_baselines
            ...
            legend_text=[legend_text,name_bs(b)];
            text_in_textbox=[text_in_textbox,...
                strcat(name_bs(b),{': RMSE [m]: '},num2str(SWH_RMSE(b,1),'%.4g'),{' , std [m]: '},num2str(SWH_std_mean(b,1),'%.4g'))];
            if ~isempty(input_path_L2_STL)
                text_errors_STL_ISR=[text_errors_STL_ISR,...
                    strcat({'Mean error '},name_bs_L2_STL,{' - '},name_bs(b),{' [m]: '},num2str(SWH_mean_error_STL_ISR(b,1),'%.4g'))];
            end
        end
        ...
        print(print_file,res_fig,strcat(path_comparison_results,data_string,'_cmp_',ext_baselines_comp,'_SWH',file_ext))
        save(strcat(path_comparison_results,data_string,'_cmp_',ext_baselines_comp,'_SWH','.mat'),...
            'input_path_L2_STL','ISR_lat_surf','STL_L2_SWH','marker_STL','color_STL','marker_ISR_ESA','color_ISR_ESA',...
        ...
        if ~isempty(input_path_L2_ISR_L1B_ESA)
            plot(ESA_lat_surf,ISR_ESA_sigma0,'Marker',marker_ISR_ESA,'Color',color_ISR_ESA);
            hold on;
            legend_text=[legend_text,name_bs_L2_ISR_L1B_ESA];
            text_in_textbox=[text_in_textbox,...
                strcat(name_bs_L2_ISR_L1B_ESA,{': RMSE [dB]: '},num2str(ISR_ESA_sigma0_RMSE,'%.4g'),{' , std [dB]: '},num2str(ISR_ESA_sigma0_std_mean,'%.4g'))];
        end
        ...
        if ~isempty(input_path_L2_ESA)
            ...
            legend_text=[legend_text,name_bs_L2_ESA];
            text_in_textbox=[text_in_textbox,...
                strcat(name_bs_L2_ESA,{' : RMSE [dB]: '},num2str(ESA_sigma0_RMSE,'%.4g'),{' , std [dB]: '},num2str(ESA_sigma0_std_mean,'%.4g'))];
        end
        ...
        if ~isempty(input_path_L2_STL)
            ...
            legend_text=[legend_text,name_bs_L2_STL];
            text_in_textbox=[text_in_textbox,...
                strcat(name_bs_L2_STL,{': RMSE [dB]: '},num2str(STL_sigma0_RMSE,'%.4g'),{' , std [dB]: '},num2str(STL_sigma0_std_mean,'%.4g'))];
        end
        ...
        for b=1:N_baselines
            ...
            legend_text=[legend_text,name_bs(b)];
            text_in_textbox=[text_in_textbox,...
                strcat(name_bs(b),{': RMSE [dB]: '},num2str(sigma0_RMSE(b,1),'%.4g'),{' , std [dB]: '},num2str(sigma0_std_mean(b,1),'%.4g'))];
            
            if ~isempty(input_path_L2_ISR_L1B_ESA)
                text_errors_ESA_ISR=[text_errors_ESA_ISR,...
                    strcat({'Mean error '},name_bs_L2_ISR_L1B_ESA,{' - '},name_bs(b),{' [dB]: '},num2str(sigma0_mean_error_ISR_ESA_ISR(b,1),'%.4g'))];
            end
            if ~isempty(input_path_L2_ESA)
                text_errors_ESA_ISR=[text_errors_ESA_ISR,...
                    strcat({'Mean error '},name_bs_L2_ESA,{' - '},name_bs(b),{' [dB]: '},num2str(sigma0_mean_error_ESA_ISR(b,1),'%.4g'))];
            end
            if ~isempty(input_path_L2_STL)
                text_errors_STL_ISR=[text_errors_STL_ISR,...
                    strcat({'Mean error '},name_bs_L2_STL,{' - '},name_bs(b),{' [dB]: '},num2str(sigma0_mean_error_STL_ISR(b,1),'%.4g'))];
            end
        ...
        print(print_file,res_fig,strcat(path_comparison_results,data_string,'_cmp_',ext_baselines_comp,'_sigma0',file_ext))
        save(strcat(path_comparison_results,data_string,'_cmp_',ext_baselines_comp,'_sigma0','.mat'),...
            'input_path_L2_ESA','input_path_L2_ISR_L1B_ESA','ESA_lat_surf','ESA_L2_sigma0_r1','marker_ESA','color_ESA','marker_ISR_ESA','color_ISR_ESA',...
        ...
        for b=1:N_baselines
            ...
            if ~isempty(input_path_L2_ESA)
                text_errors_ESA_ISR=[text_errors_ESA_ISR,...
                    strcat({'Mean error '},name_bs_L2_ESA,{' - '},name_bs(b),{' : '},num2str(nanmean(ESA_nb-nb(b,:)),'%.4g'))];
            end
        ...
        print(print_file,res_fig,strcat(path_comparison_results,data_string,'_cmp_',ext_baselines_comp,'_nb',file_ext))
        save(strcat(path_comparison_results,data_string,'_cmp_',ext_baselines_comp,'_nb','.mat'),...
            'input_path_L2_ESA','ESA_lat_surf','ESA_nb','marker_ESA','color_ESA','marker_ISR_ESA','color_ISR_ESA',...
        ...
        print(print_file,res_fig,strcat(path_comparison_results,data_string,'_cmp_',ext_baselines_comp,'_COR',file_ext))
        save(strcat(path_comparison_results,data_string,'_cmp_',ext_baselines_comp,'_COR','.mat'),...
            'N_baselines','COR','marker_bs','color_bs','marker_ISR_ESA','color_ISR_ESA',...
            'name_bs','name_bs_L2_ESA','name_bs_L2_STL','name_bs_L2_ISR_L1B_ESA','sh_name_nc','legend_text','legend_fontsize','text_in_textbox','textbox_fontsize',...
            'path_comparison_results','data_string','file_ext');
        close(f1);
    end

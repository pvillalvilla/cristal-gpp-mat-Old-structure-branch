...
function [res]=L2_validation(filename_L2_ISD,filename_L2_ESA,path_results_comparison,varargin)
    ...
    for i_ret=1:length(retrackers)
        switch char(retrackers(i_ret))
            case {'ANALYTICAL','SAMOSA'}
                ...
                % function
                % SSH
                ISD_L2_SSH_analytical_smoothed(i_wfm)   = nanmean(ISD_L2_SSH_analytical(start_SSH:finish_SSH));
                %sigma0
                ISD_L2_sigma0_analytical_smoothed(i_wfm) = nanmean(ISD_L2_sigma0_analytical(start_sigma0:finish_sigma0));
                %SWH
                ISD_L2_SWH_analytical_smoothed(i_wfm) = nanmean(ISD_L2_SWH_analytical(start_SWH:finish_SWH));
                %COR: pearson correlation coefficient
                ISD_L2_COR_analytical_smoothed(i_wfm) = nanmean(ISD_L2_COR_analytical(start_COR:finish_COR));
            case {'THRESHOLD'}
                % SSH
                ISD_L2_SSH_threshold_smoothed(i_wfm)   = nanmean(ISD_L2_SSH_threshold(start_SSH:finish_SSH));
                %sigma0
                ISD_L2_sigma0_threshold_smoothed(i_wfm) = nanmean(ISD_L2_sigma0_threshold(start_sigma0:finish_sigma0));                
           case {'OCOG_ice'}
                % SSH
                ISD_L2_SSH_OCOG_ice_smoothed(i_wfm)   = nanmean(ISD_L2_SSH_OCOG_ice(start_SSH:finish_SSH));
                %sigma0
                ISD_L2_sigma0_OCOG_ice_smoothed(i_wfm) = nanmean(ISD_L2_sigma0_OCOG_ice(start_sigma0:finish_sigma0));                                
        end
    end
end
...
if ESA_num_surfaces_filtered==ISD_num_surfaces_filtered
    ...
    if ~isempty(idx_int_analytical)
        ...
        res.SSH.ANALYTICAL.RMSE_error_L2=sqrt(nanmean((ESA_L2_SSH_r1_filtered(~idx_outliers_corr)-ISD_L2_SSH_analytical_filtered(~idx_outliers_corr)).^2));
        fprintf(fid,'RMSE error SSH ESA-ISD (analytical-retracker) [m]: '); fprintf(fid,'%.18g\n',res.SSH.ANALYTICAL.RMSE_error_L2);
        res.SSH.ANALYTICAL.mean_error_L2=(nanmean((ESA_L2_SSH_r1_filtered(~idx_outliers_corr)-ISD_L2_SSH_analytical_filtered(~idx_outliers_corr))));
    ...
    if ~isempty(idx_int_thres)
        ...
        res.SSH.THRESHOLD.RMSE_error_L2=sqrt(nanmean((ESA_L2_SSH_r1_filtered(~idx_outliers_corr)-ISD_L2_SSH_threshold_filtered(~idx_outliers_corr)).^2));
        fprintf(fid,'RMSE error SSH ESA-ISD (threshold-retracker) [m]: '); fprintf(fid,'%.18g\n',res.SSH.THRESHOLD.RMSE_error_L2);
        res.SSH.THRESHOLD.mean_error_L2=(nanmean((ESA_L2_SSH_r1_filtered(~idx_outliers_corr)-ISD_L2_SSH_threshold_filtered(~idx_outliers_corr))));
    ...
    if ~isempty(idx_int_ocog)
        ...
        res.SSH.OCOG_ice.RMSE_error_L2=sqrt(nanmean((ESA_L2_SSH_r1_filtered(~idx_outliers_corr)-ISD_L2_SSH_OCOG_ice_filtered(~idx_outliers_corr)).^2));
        fprintf(fid,'RMSE error SSH ESA-ISD (OCOG_ice-retracker) [m]: '); fprintf(fid,'%.18g\n',res.SSH.OCOG_ice.RMSE_error_L2);
        res.SSH.OCOG_ice.mean_error_L2=(nanmean((ESA_L2_SSH_r1_filtered(~idx_outliers_corr)-ISD_L2_SSH_OCOG_ice_filtered(~idx_outliers_corr))));
...
if ~isempty(idx_int_analytical)
    fprintf(fid,'$---------------- ANALYTICAL RETRACKER -------------------------------------------$\n');
    res.SSH.ANALYTICAL.mean_error_fitting=nanmean(ISD_L2_SSH_analytical(ISD_indexes_int)-ISD_L2_SSH_analytical_smoothed);
    fprintf(fid,'Mean error SSH fitting (analytical-retracker) [m]: '); fprintf(fid,'%.18g\n',res.SSH.ANALYTICAL.mean_error_fitting);
    res.SSH.ANALYTICAL.rmse_fitting=sqrt(nanmean((ISD_L2_SSH_analytical(ISD_indexes_int)-ISD_L2_SSH_analytical_smoothed).^2));
    fprintf(fid,'RMSE SSH fitting (analytical-retracker) [m]: '); fprintf(fid,'%.18g\n',res.SSH.ANALYTICAL.rmse_fitting);
end
if ~isempty(idx_int_thres)
    fprintf(fid,'$---------------- THRESHOLD  RETRACKER -------------------------------------------$\n');
    res.SSH.THRESHOLD.mean_error_fitting=nanmean(ISD_L2_SSH_threshold(ISD_indexes_int)-ISD_L2_SSH_threshold_smoothed);
    fprintf(fid,'Mean error SSH fitting (threshold-retracker) [m]: '); fprintf(fid,'%.18g\n',res.SSH.THRESHOLD.mean_error_fitting);
    res.SSH.THRESHOLD.rmse_fitting=sqrt(nanmean((ISD_L2_SSH_threshold(ISD_indexes_int)-ISD_L2_SSH_threshold_smoothed).^2));
    fprintf(fid,'RMSE SSH fitting (threshold-retracker) [m]: '); fprintf(fid,'%.18g\n',res.SSH.THRESHOLD.rmse_fitting);
end
if ~isempty(idx_int_ocog)
    fprintf(fid,'$---------------- OCOG_ICE   RETRACKER -------------------------------------------$\n');
    res.SSH.OCOG_ice.mean_error_fitting=nanmean(ISD_L2_SSH_OCOG_ice(ISD_indexes_int)-ISD_L2_SSH_OCOG_ice_smoothed);
    fprintf(fid,'Mean error SSH fitting (OCOG_ice-retracker) [m]: '); fprintf(fid,'%.18g\n',res.SSH.OCOG_ice.mean_error_fitting);
    res.SSH.OCOG_ice.rmse_fitting=sqrt(nanmean((ISD_L2_SSH_OCOG_ice(ISD_indexes_int)-ISD_L2_SSH_OCOG_ice_smoothed).^2));
    fprintf(fid,'RMSE SSH fitting (OCOG_ice-retracker) [m]: '); fprintf(fid,'%.18g\n',res.SSH.OCOG_ice.rmse_fitting);
end

fprintf(fid,'$---------------- ESA RETRACKER -------------------------------------------$\n');
res.SSH.ESA_L2.mean_error_fitting=nanmean(ESA_L2_SSH_r1_filtered(~idx_outliers_corr)-ESA_L2_SSH_r1_smoothed(~idx_outliers_corr));
fprintf(fid,'Mean error SSH fitting (L2-ESA) [m]: '); fprintf(fid,'%.18g\n',res.SSH.ESA_L2.mean_error_fitting);
res.SSH.ESA_L2.rmse_fitting=sqrt(nanmean((ESA_L2_SSH_r1_filtered(~idx_outliers_corr)-ESA_L2_SSH_r1_smoothed(~idx_outliers_corr)).^2));
...
[counts,centers_ESA_L2_SSH_r1_filtered]=hist(ESA_L2_SSH_r1_filtered(~idx_outliers_corr),nbins);
...
%------------------------ ESA ---------------------------------------------
figure; plot(ESA_lat_surf(ESA_indexes_int),ESA_L2_SSH_r1(ESA_indexes_int),linestyle_ESA);
hold on;
...
if ESA_num_surfaces_filtered==ISD_num_surfaces_filtered
    ...
    if ~isempty(idx_int_analytical)
        ...
        res.SIGMA0.ANALYTICAL.RMSE_error_L2=sqrt(nanmean((ESA_L2_sigma0_r1_filtered(~idx_outliers_corr)-ISD_L2_sigma0_analytical_filtered(~idx_outliers_corr)).^2));
        fprintf(fid,'RMSE error sigma0 ESA-ISD (analytical-retracker) [dB]: '); fprintf(fid,'%.18g\n',res.SIGMA0.ANALYTICAL.RMSE_error_L2);
        res.SIGMA0.ANALYTICAL.mean_error_L2=(nanmean((ESA_L2_sigma0_r1_filtered(~idx_outliers_corr)-ISD_L2_sigma0_analytical_filtered(~idx_outliers_corr))));
    ...
    if ~isempty(idx_int_thres)
        ...
        res.SIGMA0.THRESHOLD.RMSE_error_L2=sqrt(nanmean((ESA_L2_sigma0_r1_filtered(~idx_outliers_corr)-ISD_L2_sigma0_threshold_filtered(~idx_outliers_corr)).^2));
        fprintf(fid,'RMSE error sigma0 ESA-ISD (threshold-retracker) [dB]: '); fprintf(fid,'%.18g\n',res.SIGMA0.THRESHOLD.RMSE_error_L2);
        res.SIGMA0.THRESHOLD.mean_error_L2=(nanmean((ESA_L2_sigma0_r1_filtered(~idx_outliers_corr)-ISD_L2_sigma0_threshold_filtered(~idx_outliers_corr))));
    ...
    if ~isempty(idx_int_ocog)
        ...
        res.SIGMA0.OCOG_ice.RMSE_error_L2=sqrt(nanmean((ESA_L2_sigma0_r1_filtered(~idx_outliers_corr)-ISD_L2_sigma0_OCOG_ice_filtered(~idx_outliers_corr)).^2));
        fprintf(fid,'RMSE error sigma0 ESA-ISD (OCOG_ice-retracker) [dB]: '); fprintf(fid,'%.18g\n',res.SIGMA0.OCOG_ice.RMSE_error_L2);
        res.SIGMA0.OCOG_ice.mean_error_L2=(nanmean((ESA_L2_sigma0_r1_filtered(~idx_outliers_corr)-ISD_L2_sigma0_OCOG_ice_filtered(~idx_outliers_corr))));
...
fprintf(fid,'$---------------- ESA RETRACKER -------------------------------------------$\n');
res.SIGMA0.ESA_L2.mean_error_fitting=nanmean(ESA_L2_sigma0_r1_filtered(~idx_outliers_corr)-ESA_L2_sigma0_r1_smoothed(~idx_outliers_corr));
fprintf(fid,'Mean error sigma0 fitting (L2-ESA) [dB]: '); fprintf(fid,'%.18g\n',res.SIGMA0.ESA_L2.mean_error_fitting);
res.SIGMA0.ESA_L2.rmse_fitting=sqrt(nanmean((ESA_L2_sigma0_r1_filtered(~idx_outliers_corr)-ESA_L2_sigma0_r1_smoothed(~idx_outliers_corr)).^2));

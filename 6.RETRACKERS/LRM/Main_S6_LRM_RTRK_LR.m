...
cnf_p.MainPath          = ['C:\Users\albert\Documents\WORK\S6GPP\DATA\Euribia\output_data\',lower(MODE),'\'];
...
%% DATA STRUCTURE
% -----------------------------------------------------------------
data.LAT        =   double(ncread([cnf_p.MainPath,cnf_p.Filename],['/data_20/',BAND,'/latitude']));
data.LON        =   double(ncread([cnf_p.MainPath,cnf_p.Filename],['/data_20/',BAND,'/longitude']));
data.H      	=   double(ncread([cnf_p.MainPath,cnf_p.Filename],['/data_20/',BAND,'/altitude']));
data.WD         =   double(ncread([cnf_p.MainPath,cnf_p.Filename],['/data_20/',BAND,'/tracker_range_calibrated']));

data.Re         =   sqrt(nf_p.semi_major_axis^2*cos(data.LAT).^2+nf_p.semi_minor_axis^2*sin(data.LAT).^2);
data.pitch      =   double(ncread([cnf_p.MainPath,cnf_p.Filename],['/data_20/',BAND,'/off_nadir_pitch_angle_pf']));
...
sigma0_sc_ft    =   double(ncread([cnf_p.MainPath,cnf_p.Filename],['/data_20/',BAND,'/sig0_scaling_factor']));
...
[~,~,SSH_mean_std,SSH_mean_bias,del_ind]        = block_statistics([out.SSH],win_size_stat);

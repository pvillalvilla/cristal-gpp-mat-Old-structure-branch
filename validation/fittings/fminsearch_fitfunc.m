...
function [x,fval,exitflag,output] = fminsearch_fitfunc(funfcn,x,options,varargin)
...
usual_delta = 1;           % 5 percent deltas for non-zero terms
zero_term_delta = 1;   % Even smaller delta for zero elements of x
for j = 1:n
   y = xin;
   if y(j) ~= 0

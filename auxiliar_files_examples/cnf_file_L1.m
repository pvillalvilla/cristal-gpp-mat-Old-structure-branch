...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
cnf.mode            ='SIN';
cnf.processing_mode ='SIN';
cnf.verify_L0   = 0;
cnf.verify_L1A  = 0;
cnf.verify_L1BS_internal  = 1;
cnf.verify_L1B = 1;
% CHAINS
cnf.writting_flag = [0 0 1 0 0 0 1]; % L1A, L1BS_HR, L1B_HR, L1B_VHR_time, L1B_VHR_frequency, LROS, KML
cnf.run_L1A     = 0;
cnf.run_DDP     = 1;
cnf.run_FFtime  = 0;
cnf.run_FFfreq  = 0;
...
% Flag to know if we start processing from a L0 netCDF file
cnf.starting_from_netcdf=1;
...
%% ONBOARD REVERSION
cnf.onboard_reversion_flag = 0; % JPLZ: I added it and set it to 0, it was not appearing and the L1A chain looks for it 
...
%test for correct processing sea ice scenario
% cnf.window_rg = 0;
% chd.N_samples=256; %number sar samples for window calculation below
% cnf.flag_range_win_type='Boxcar';%{'Boxcar','Hamming','Hanning','Kaiser','Blackman'}
% cnf.range_BW_to_process=395e6; % in Hz;
% cnf.range_BW_to_process=600e6; % in Hz;
% cnf.flag_range_win_param=5; %parameter of the window: for Kaiser 
% cnf.flag_range_win_normalize=1;
% cnf.flag_range_win_normalize_type=0;
...
cnf.figures_visible=1;

%% Configuration parameters FFt
% For testing goals, force a small number of surfaces to be computed for fast processing
cnf.FFt.force_max_N_surf = 0;
cnf.FFt.max_N_surf = 200;%0;%100;%300;

% Computation grid
cnf.FFt.spacing_al_surfaces = 0.5; %spacing between the surfaces along-track
cnf.FFt.out_spatial_res = 10; %defines the spatial resolution over which to perform the incoherent multilooking of the FF waveforms (related to HR-DDP resolution)
% Multilook alignment options
cnf.FFt.ref_alignment_ML = 1;

% Kernel of backprojections
cnf.FFt.antenna_pattern_impact_flag = 0; % compensate the effect of antenna pattern (to be implemented when paralelizing)
cnf.FFt.T_integration = 2.2; % %if set to -1 take all the input data for processing
cnf.FFt.T_integration = 2.3; % %if set to -1 take all the input data for processing
cnf.FFt.num_pools = 20; % to paralelize the processing of each surface
...
cnf.FFt.force_POI= 1; % JPLZ: Added it, FFt algorithm was looking for this key
cnf.rmc_margin = double.empty; % JPLZ: Added it, the ML part needs it (it's empty in S6 code)

% Leads
cnf.RMC_RMC_on = 1;

cnf.RMC.window_sizes= 1;

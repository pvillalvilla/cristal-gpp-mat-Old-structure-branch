...
% Version  record
% 1.0 2018/07/12 
% 2.0 2019/06/10 Added multiple parameter for new modes
% 
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
cnf.mode            ='SAR';
...
cnf.verify_L1B = 0;
% CHAINS
% L1A, L1BS_HR, L1B_HR, L1B_VHR_time, L1B_VHR_frequency, LROS, L2GR, KML
cnf.writting_flag = [1 0 1 0 0 0 0 1]; 
cnf.run_L1A     = 1;
cnf.run_DDP     = 1;
cnf.run_FFtime  = 0;
cnf.run_FFfreq  = 0;
...
cnf.flag_range_win_param = 0.5; %parameter of the window: for Kaiser 

cnf.flag_range_win_normalize = 1; %avoid any change on sigma0 scaling factor
cnf.flag_range_win_normalize_type = 0; % {0: energy, 1: amplitude}
...
cnf.FFt.zp_ac_TRP = 32; % For transponder: zero-padding for across-track over the final image in TRP case
cnf.FFt.zp_al_TRP = 32; % For transponder: zero-padding for across-track over the final image in TRP case

%% Config Parameters for RMC Analysis
cnf.RMC.RMC_on = 1;
cnf.RMC.tracking_gate = 272;
cnf.RMC.window_sizes = [32 64];
cnf.RMC.pwr_thresh = 10;
cnf.RMC.sea_ice_power_thresh = 10^(-135/10);
cnf.RMC.prominence_thresh = 3;
cnf.RMC.width_thresh = 1;
cnf.RMC.coherence_thresh = 0.7;
